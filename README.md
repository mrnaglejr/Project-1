...
- *How does Bitcoin price and performance and volatility compare to equity markets?*
...
and comparison of Bitcoin's historical price performance and volatility to 3 ETF's (SPY, DIA, QQQ) and 3 high-performing stocks (AAPL, AMZN, MSFT):
...
Our rationale for using a mix of ETF's and a few individual stocks was to capture both the general trend of the broader equity market, as well as the peaks and valleys of top performers.
...
- Examine and compare historical returns and volatility of index ETFs, stocks, and Bitcoin over full period for which we have Bitcoin prices. Assess BTC performance and volatility compared to other entities over the same period.

- Examine and compare historical returns and volatility of index ETFs, stocks, and Bitcoin across each entity's first 3,000 trading days. Assess BTC's early performance and volatility compared to early performance and volatility of other entities in comparison.
...
- Import historical stock prices for Apple (AAPL), Amazon(AMZN), Microsoft (MSFT), S&P 500 (SPY), Dow Jones (DIA), QQQ Trust Series Fund (QQQ) ino Pandas dataframes using the Yahoo Finance API, and exported the historical prices into individual csv files.
...
- Calculate daily and annualized standard deviations for each entity over the analysis period. Plot the standard deviations for comparison
- To assess early price performance and volatility, create new dataframes consisting of each entity's earliest 3,000 trading days.
...
- Imported csv containing BTC historical closing prices and daily returns, load into a Pandas dataframe, and format to be compatible with MCForecastTools.py script.
- Set up 1,000 path, 10-year Monte Carlo simulations for all entities.
- Run Monte Carlo simulations for all entities.
- Plot simulation paths and distrbution of forecasted returns.
- Summarize the Monte Carlo forecast results.

## **Analysis Results**

### ***How does BTC price perfomance compare to equity markets?***

To start our analysis, we wanted to compare Bitcoin's price performance from 2013 to present day against the ETF's and stocks. We calculated the daily returns and plotted them:

![Daily Returns](https://github.com/mrnaglejr/Project-1/blob/main/images/daily_returns_full.png)

The plot of the daily returns highlighted both the volatility of Bitcoin. While it showcased explosive growth unmatched by the other entities, Bitcoin also experienced some violent corrections over the period.
...
On the whole, the plot of the cumulative returns showed that Bitcoin has widely outperformed the equities in our comparison over the period.Additionally, the cumulative returns plot further highlight the wild swings of BTC. We see periods of explosive growth, followed by severe corrections.
...
The box plot tells the same story that we saw with the daily returns plot. Bitcoin clearly has a much wider spread of daily returns, featuring both outsized growth and declines. 

The next dimension of out volatility analysis consisted of a comparison of daily and annualized standard deviations. We plotted both the daily and annualized standard deviations of each entity:
...
Our initial analysis showed that while BTC has produced greater returns than the ETFs and stocks in the comparison, it was also a highly volatile asset that has experienced multiple crashes. The next phase of our analysis was geared at determining whether BTC's explosive growth and extreme volatility was unique, or more typical of a new asset or asset class. In other words, is Bitcoin an outlier?
...
However, looking at cumulative returns over the first 3,000 trading days, BTC still appears to outperform all other entities by a meaningful margin:
...
Overall, the analysis of the early returns history shows that Bitcoin does appear to be outlier in terms of cumulative returns. Moreover, the analysis also shows that Bitcoin's return volatility is not necessarily unprecedented; both Apple and Amazon stock observed comparable price swings in their early history. 

### *How does BTC's volatility perforance compare to the early volatility of equities?*

A box plot of the daily returns across the first 3,000 trading days emphasizes the point that BTC's return volatility is takeaway from above:
...
The comparison of early standard deviations further supports this sentiment:
...
Again, Bitcoin is near the top in terms of daily and annualized standard deviations. Both Apple and Amazon's standard deviations across their first 3,000 trading days appear to be very comparable. In fact, both stocks' annualized standard deviations appear to exceed Bitcoin's annualized standard deviation. In short, the comparison of early price history suggests that Bitcoin's volatility is not necessarily a significant outlier.
...
The results of the Monte Carlo simulation suggest that an investment in Bitcoin is a relatively good bet for the future, especially for an investor with high risk tolerance. A $10,000 investment in Bitcoin has the highest upper range of the 95% confidence interval ($59M) and the highest median expected value ($957K). It also has the 4th highest lower range of the 95% confidence interval, representing a total 10-year return of roughly 40% ($14K). In other words, the MC forecast suggests that based on the observed price history, you are unlikely to lose money on a 10-year investment in BTC. Additionally, the forecast suggests there is a chance for significant upside.

That said, the wide spread between the upper and lower ranges of the 95% confidence intervals further highlight the extremes of investing in Bitcoin, and perhaps the crytpo market more broadly.
...
- Results of MC simulations suggest that BTC has a good chance to continue to at least be reasonably profitable, with potential for considerable upside:
    - Highest median expected return, highest upper range of 95% confidence interval, 4th highest lower range of 95% confidence interval

## **Other Considerations**

Below are other considerations aside from the historical price performance and future projections that could be weighed before investing in BTC:

- **Regulatory Environment**: Will governments be friendly to BTC and accomodative of the broader crypto market?
- **Environmental Concerns**: Will recent concerns over BTC energy consumption cause investors to shy away, or open the door for more eco-conscious cryptos to take control?
- **Nascency of Crypto Market**: BTC is the first mover and current leader, but will it remain on top in an increasingly competitive space?
